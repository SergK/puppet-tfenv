  ...
  context 'with defaults for all parameters' do
    ...
    it {
      [
        ...
        is_expected.to contain_package(fn).with({
          'ensure' => 'latest'})
      end
    }
    ...
    it { is_expected.to contain_file('/opt/tfenv').with({
        'ensure'  => 'directory',
        'owner'   => 'jenkins',
        'group'   => 'jenkins',
        'mode'    => '0755',
...
Package[unzip]{:name=>\"unzip\"}]"})
    ...
    it { is_expected.to contain_file('/usr/local/bin/tfenv').with({
        'ensure'  => 'link',
    ...
    it { is_expected.to contain_file('/usr/local/bin/terraform').with({
        'ensure'  => 'link',
    ...
    it { is_expected.to contain_vcsrepo('/opt/tfenv').with({
      'ensure'   => 'present',
      'provider' => 'git',
      'revision' => 'v0.5.2',
